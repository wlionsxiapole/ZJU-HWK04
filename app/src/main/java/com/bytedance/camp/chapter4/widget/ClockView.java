...
public class ClockView extends View {
    ...
    private void drawTimeNeedles(Canvas canvas) {
         ...
         */
        String TMIELOG = "TIME";
        Log.i(TMIELOG,"CurrentTime: hour:"+hour+"  minute:"+minute+"  second:"+second);
        needlePaint.setStrokeWidth(HOUR_NEEDLE_WIDTH);
        int hourDegree = 30 * hour - 90;
        float endHX = (float)(centerX + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(hourDegree)));
        float endHY = (float)(centerY + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(hourDegree)));
        canvas.drawLine(centerX,centerY,endHX,endHY,needlePaint);

        needlePaint.setStrokeWidth(MINUTE_NEEDLE_WIDTH);
        int minuteDegree = 6 * minute - 90;
        float endMX = (float)(centerX + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(minuteDegree)));
        float endMY = (float)(centerY + radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(minuteDegree)));
        canvas.drawLine(centerX,centerY,endMX,endMY,needlePaint);

        needlePaint.setStrokeWidth(SECOND_NEEDLE_WIDTH);
        int secondDegree = 6 * second - 90;
        float endSX = (float)(centerX + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(secondDegree)));
        float endSY = (float)(centerY + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(secondDegree)));
        canvas.drawLine(centerX,centerY,endSX,endSY,needlePaint);
        // int hourDegree = 180;
